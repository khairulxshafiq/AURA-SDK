...
* **Bare URL Instant Scrape Routing & Callback Fix**: Updated `route_intent` in `ui/telegram_bot.py` to route bare URL pastes directly to `"SCRAPE_PIPELINE"`, executing scrape, extracting image, and returning photo preview with inline keyboards immediately without requiring manual keywords. Added `do_scrape:` and `do_summarize:` callback handlers in `handle_callback_query`.
* **Airtable Attachment E2E Fallback Lock & Instant Schema Sync**: Updated `confirm_platform` in `ui/telegram_bot.py` to fall back to `draft.get("image_url")` (`https://...`) when `telegram_direct_cdn_url` is missing, guaranteeing physical image attachment push to Airtable Content Station. Optimized default payload in `tools/publisher_service.py` to match Airtable Content Station schema (`"Image file"` field), eliminating schema retry latency.
* **Google News RSS Resolution & HTTPS Normalization**: Installed `googlenewsdecoder` package and added it to `requirements.txt` to decode Google News RSS article URLs (`news.google.com/rss/articles/CBM...`) directly to actual publisher URLs. Implemented `_ensure_https(url_str)` in `tools/web_scraper.py`, `ui/formatters.py`, and `ui/telegram_bot.py` to upgrade `http://` image URLs to secure `https://` URLs, preventing Telegram API `sendPhoto` rejection and Airtable attachment ingestion failure.
* **Scraper Image Extraction (4-Tier Priority)**: Implemented `extract_article_image_url(soup, page_url)` in `tools/web_scraper.py` enforcing 4-tier priority sequence (`og:image` → `twitter:image` → `article hero image` → `first valid article image`) and returning `article_image_url`. Updated `_scrape_native`, `_scrape_firecrawl`, and `_scrape_jina` to return `article_image_url` and `image_url`.
* **Live Search Routing Rule**: Implemented mandatory `LIVE_NEWS_SEARCH` router in `ui/telegram_bot.py` matching 12 keywords (`berita`, `berita terkini`, `current news`, `headline`, `trending`, `viral`, `cerita menarik`, `cerita semasa`, `apa berlaku hari ini`, `top stories`, `trending malaysia`, `trending dunia`). Bypassed memory-first responses to fetch fresh live articles before drafting content. Added `fetch_live_news_with_fallback` in `tools/search_engine.py` implementing 3-tier priority fetch chain (Tier 1: GNews → Tier 2: Internet Search → Tier 3: RSS Feeds).
* **Telegram Inline Keyboard Rules & Keyboard Validation**: Built `ui/keyboard_validator.py` to validate `InlineKeyboardMarkup` objects prior to rendering or message dispatch (uniqueness of `callback_data`, handler pattern mapping, state update mapping). Added explicit `hashtag_on` (`✅ Dengan Hashtag #️⃣`) and `hashtag_off` (`🚫 Tanpa Hashtag`) buttons to `_get_sub_options_keyboard` in `ui/keyboards.py`. Handled `hashtag_on` and `hashtag_off` in `handle_callback_query` inside `ui/telegram_bot.py`, saving state to `draft_repo` and calling `query.edit_message_reply_markup` & re-triggering draft generation preview immediately without waiting for confirmation.
* **AURA-Trade (Trading Sub-Crew Engine) Verified**: Verified 7 deterministic function-calling tools in `tools/trading_service.py` using top-down analysis, RSI-14, ATR-14, MA-20/50/200, fundamental ratios (P/E, NTA, ROE, Dividend Yield), position sizing, and Mode Swing vs Position / DCA analysis.
* **Automated Unit Testing & Consolidated Git Release**: Added automated test suites: `ui/test_keyboards.py`, `tools/test_live_search.py`, `tools/test_image_extraction.py` (13/13 tests passing 100%). Consolidated features and merged into `main`, pushed live to `origin/main`.
...
* **AURA v5 Threads & X Persona Prompts Engine Integrated**: Created `orchestrator/threads_x_personas.py` containing Threads thread-count guides (3, 5, 8 threads separated by `---`) and 5 shared writing styles (`catchy`, `genz`, `informative`, `hook_memanggil`, `affiliate`), along with X (Twitter) mini-thread constraints (<= 280 chars per tweet). Integrated `build_threads_prompt()` and `build_x_prompt()` into `_call_draft_generator_model()` in `ui/telegram_bot.py`.
* **AURA v5 FB Sub-Platform Persona Prompts Integrated**: Created `orchestrator/fb_personas.py` containing 6 specialized FB persona prompts: `fb_berita` (Dateline, neutral journalist), `fb_pemerhati` (Reflective, inspirational observer), `fb_kedai_kopi` (Mamak talk, opinionated logic), `fb_viral_santai` (Fun, relatable, punchy), `fb_makcik_bawang` (Dramatic gossip, curious), and `fb_kisah_inspirasi` (Heartwarming, motivational). Integrated `build_fb_prompt()` into `_call_draft_generator_model()` in `ui/telegram_bot.py`.
* **Major Intent Router Reset Implemented**: Resolved routing bug where general conversational chat ("Hi aura!", "Kau ok?") triggered the draft content pipeline with `📰 MASTER ARTICLE` headers and Inline Keyboards. Updated `handle_message` in `ui/telegram_bot.py` to completely isolate general conversational chat (replying with clean Markdown conversational text without keyboards) from Scrape / URL / Command intents (`http://`, `https://`, `/s1`, `/s2`, `scrape`), which exclusively drive the 4-step Content Pipeline State Machine.
* **Clean Platform Draft Output & Direct Photo Delivery Standardized**: Guaranteed 100% clean, pure caption output for all platform drafts (FB/X/Threads/Lemon8). Updated `subagents/social_agent.py` and `ui/telegram_bot.py` with strict system instructions and `_clean_platform_draft_output()` post-processor to automatically strip conversational intros, visual/GIF recommendations, and structural header labels (`FACEBOOK POST:`, `Kapsyen:`). Verified Telegram Direct Photo delivery with cached `file_id` for Airtable attachment ingestion.
* **Dynamic Master Article Variable Extraction Implemented**: Fixed issue where Telegram Master Article output displayed static placeholder text (`"Teks Master Article neutral lengkap di sini..."`). Updated `_process_response_draft` in `ui/formatters.py` to dynamically extract the full LLM-generated body text from `raw_body` (stripping trailing metadata tags), and removed static placeholder lines from `ui/telegram_bot.py`'s direct scrape pipeline prompt template.
* **Master Article Neutral Core Context Engine Standardized**: Re-architected Master Article generation in `subagents/social_agent.py`, `ui/telegram_bot.py` (`_execute_direct_scrape_pipeline`), and `orchestrator/persona.txt` from marketing/conversational tone to **Neutral Core Context & Story Hub**. Enforced style-free format (prohibiting blog intros, conversational openers, hashtags, and CTAs) with 4 mandatory sections: 📌 Tajuk Asal / Fokus Utama, 📝 Ringkasan Isu / Ringkasan Cerita, 📊 Fakta & Poin Penting, and 💡 Sudut Pandang Kunci.
* **Two-Step Delivery Strategy Implemented**: Resolved silent crashes and hanging caused by Telegram's 1,024-character image caption limit. Updated `_process_response_draft` in `ui/formatters.py` to separate preview photo delivery (`send_photo` with short caption `< 50` chars) from text delivery (`reply_text` with full Master Article + Inline Keyboards up to 3,500 chars). Wrapped Telegram API calls in robust `try-except` blocks with plain text fallback, eliminating caption length overflow crashes.
* **Master Article Display & Formatting Fix**: Resolved issue where Master Article body text was stripped out by regex while leaving conversational intro text (*"Tentu, berikut adalah Master Article..."*). Updated `_process_response_draft` in `ui/formatters.py` to construct and inject the full Master Article body directly into Telegram text output (`📰 MASTER ARTICLE (DRAFT GENERAL SAKLUMA)` + Title + Full Master Article Body + Hashtags), completely eliminating conversational intro text.
* **Telegram Direct CDN Link Standardized (Single Source of Truth)**: Established `https://api.telegram.org/file/bot<BOT_TOKEN>/<FILE_PATH>` as the 100% exclusive single source of truth for Airtable attachment ingestion. Updated `_prepare_drive_image_for_airtable()` in `tools/publisher_service.py` to generate Telegram CDN Direct File Links directly from cached `telegram_file_id`, ensuring Airtable seamlessly ingests raw `.jpg` binary image streams into physical attachment thumbnails (`web-*.jpg`).
